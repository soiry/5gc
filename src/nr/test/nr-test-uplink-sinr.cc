...
NS_LOG_COMPONENT_DEFINE ("NrUplinkSinrTest");
 ...
 * TestSuite
 */
NrUplinkSinrTestSuite::NrUplinkSinrTestSuite ()
  : TestSuite ("nr-uplink-sinr", SYSTEM)
{
  /**
  ...
  AddTestCase (new NrUplinkDataSinrTestCase (rxPsd1, rxPsd2, theoreticalSinr1, "sdBm = [-46 -inf] and [-inf -48]"), TestCase::QUICK);
  
  AddTestCase (new NrUplinkSrsSinrTestCase (rxPsd1, rxPsd2, theoreticalSinr1, "sdBm = [-46 -inf] and [-inf -48]"), TestCase::QUICK);
  ...
  AddTestCase (new NrUplinkDataSinrTestCase (rxPsd3, rxPsd4, theoreticalSinr2, "sdBm = [-63 -inf] and [-inf -61]"), TestCase::QUICK);
  
  AddTestCase (new NrUplinkSrsSinrTestCase (rxPsd3, rxPsd4, theoreticalSinr2, "sdBm = [-63 -inf] and [-inf -61]"), TestCase::QUICK);

}

static NrUplinkSinrTestSuite nrUplinkSinrTestSuite;
...
NrUplinkDataSinrTestCase::NrUplinkDataSinrTestCase (Ptr<SpectrumValue> sv1, Ptr<SpectrumValue> sv2, Ptr<SpectrumValue> sinr, std::string name)
  : TestCase ("SINR calculation in uplink data frame: " + name),
    ...
    m_expectedSinr (sinr)
{
  NS_LOG_INFO ("Creating NrUplinkDataSinrTestCase");
}

NrUplinkDataSinrTestCase::~NrUplinkDataSinrTestCase ()
{
}


void
NrUplinkDataSinrTestCase::DoRun (void)
{
  /**
  ...
  */
  ...
  Simulator::Schedule (ts, &NrSpectrumPhy::StartRx, ulPhy, sp1);

  Ptr<NrSpectrumSignalParametersDataFrame> sp2 = Create<NrSpectrumSignalParametersDataFrame> ();
  sp2->psd = m_sv2;
  sp2->txPhy = 0;
  sp2->duration = ds;
  sp2->packetBurst = packetBursts[1];
  sp2->cellId = pbCellId[1];
  Simulator::Schedule (ts, &NrSpectrumPhy::StartRx, ulPhy, sp2);
  ...
  Simulator::Schedule (ti1, &NrSpectrumPhy::StartRx, ulPhy, ip1);
  ...
  Simulator::Schedule (ti2, &NrSpectrumPhy::StartRx, ulPhy, ip2);
  ...
  Simulator::Schedule (ti3, &NrSpectrumPhy::StartRx, ulPhy, ip3);
  ...
  Simulator::Schedule (ti4, &NrSpectrumPhy::StartRx, ulPhy, ip4);
...
NrUplinkSrsSinrTestCase::NrUplinkSrsSinrTestCase (Ptr<SpectrumValue> sv1, Ptr<SpectrumValue> sv2, Ptr<SpectrumValue> sinr, std::string name)
  : TestCase ("SINR calculation in uplink srs frame: " + name),
    ...
    m_expectedSinr (sinr)
{
  NS_LOG_INFO ("Creating NrUplinkSrsSinrTestCase");
}

NrUplinkSrsSinrTestCase::~NrUplinkSrsSinrTestCase ()
{
}

void
NrUplinkSrsSinrTestCase::ReportSinr (const SpectrumValue& sinr)
{
  m_actualSinr = sinr.Copy ();
}

void
NrUplinkSrsSinrTestCase::DoRun (void)
{
  /**
  * Instantiate a single receiving NrSpectrumPhy
  */
  ...
  Ptr<NrChunkProcessor> chunkProcessor = Create<NrChunkProcessor> ();
  chunkProcessor->AddCallback (MakeCallback (&NrUplinkSrsSinrTestCase::ReportSinr, this));
  ...
  *  NrSpectrumSignalParametersUlSrsFrame of the first signal will have the
  ...
  */
  ...
  // 2 UEs send data to the eNB through 2 subcarriers
  Ptr<NrSpectrumSignalParametersUlSrsFrame> sp1 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  sp1->psd = m_sv1;
  sp1->txPhy = 0;
  sp1->duration = ds;
  sp1->cellId = pbCellId[0];
  ...
  Ptr<NrSpectrumSignalParametersUlSrsFrame> sp2 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  sp2->psd = m_sv2;
  sp2->txPhy = 0;
  sp2->duration = ds;
  sp2->cellId = pbCellId[1];
  ...
  Ptr<NrSpectrumSignalParametersUlSrsFrame> ip1 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  ip1->psd = i1;
  ip1->txPhy = 0;
  ip1->duration = di1;
  ip1->cellId = pbCellId[2];
  ...
  Ptr<NrSpectrumSignalParametersUlSrsFrame> ip2 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  ip2->psd = i2;
  ip2->txPhy = 0;
  ip2->duration = di2;
  ip2->cellId = pbCellId[3];
  ...
  Ptr<NrSpectrumSignalParametersUlSrsFrame> ip3 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  ip3->psd = i3;
  ip3->txPhy = 0;
  ip3->duration = di3;
  ip3->cellId = pbCellId[4];
  ...
  Ptr<NrSpectrumSignalParametersUlSrsFrame> ip4 = Create<NrSpectrumSignalParametersUlSrsFrame> ();
  ip4->psd = i4;
  ip4->txPhy = 0;
  ip4->duration = di4;
  ip4->cellId = pbCellId[5];
