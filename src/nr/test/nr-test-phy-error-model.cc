...
#include "nr-test-phy-error-model.h"

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("NrTestPhyErrorModel");

LenaTestPhyErrorModelSuite::LenaTestPhyErrorModelSuite ()
  : TestSuite ("nr-phy-error-model", SYSTEM)
{
...
LenaDataPhyErrorModelTestCase::DoRun (void)
{
  ...
  double ber = 0.03;
  Config::SetDefault ("ns3::NrAmc::Ber", DoubleValue (ber));
  ...
  for (int i = 0; i < m_nEnb; i++)
    {
      ...
      mm->SetPosition (Vector (0.0, 0.0, 30.0));
      Ptr<NrEnbNetDevice> nrEnbDev = enbDevs.Get (i)->GetObject<NrEnbNetDevice> ();
      Ptr<NrEnbPhy> enbPhy = nrEnbDev->GetPhy ();
      enbPhy->SetAttribute ("TxPower", DoubleValue (43.0));
      enbPhy->SetAttribute ("NoiseFigure", DoubleValue (5.0));
    }
