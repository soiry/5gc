...
NS_LOG_COMPONENT_DEFINE ("NgcTestN2uDownlink");
...
class NgcN2uDlTestCase : public TestCase
{
public:
  NgcN2uDlTestCase (std::string name, std::vector<EnbDlTestData> v);
  virtual ~NgcN2uDlTestCase ();
...
NgcN2uDlTestCase::NgcN2uDlTestCase (std::string name, std::vector<EnbDlTestData> v)
  : TestCase (name),
    m_enbDlTestData (v)
{
}

NgcN2uDlTestCase::~NgcN2uDlTestCase ()
{
}

void 
NgcN2uDlTestCase::DoRun ()
{
  ...
  for (std::vector<EnbDlTestData>::iterator enbit = m_enbDlTestData.begin ();
       ...
       ++enbit)
    {
      ...
      // we test NGC without NR, hence we use:
      ...
      // Note that the NgcEnbApplication won't care of the actual NetDevice type
      ngcHelper->AddEnb (enb, enbDevice, cellId);      

      // Plug test RRC entity
      Ptr<NgcEnbApplication> enbApp = enb->GetApplication (0)->GetObject<NgcEnbApplication> ();
      NS_ASSERT_MSG (enbApp != 0, "cannot retrieve NgcEnbApplication");
      Ptr<NgcTestRrc> rrc = CreateObject<NgcTestRrc> ();
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          uint64_t imsi = u+1;
          ngcHelper->AddUe (ueNrDevice, imsi);
          ngcHelper->ActivateEpsBearer (ueNrDevice, imsi, NgcTft::Default (), EpsBearer (EpsBearer::NGBR_VIDEO_TCP_DEFAULT));
          enbApp->GetN2SapProvider ()->InitialUeMessage (imsi, (uint16_t) imsi);
        } 
 ...
 */
class NgcN2uDlTestSuite : public TestSuite
{
public:
  NgcN2uDlTestSuite ();
  
} g_ngcN2uDlTestSuiteInstance;

NgcN2uDlTestSuite::NgcN2uDlTestSuite ()
  : TestSuite ("ngc-n2u-downlink", SYSTEM)
{  
  ...
  e1.ues.push_back (f1);
  v1.push_back (e1);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 1UE", v1), TestCase::QUICK);
  ...
  v2.push_back (e2);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 2UEs", v2), TestCase::QUICK);


  std::vector<EnbDlTestData> v3;  
  v3.push_back (e1);
  v3.push_back (e2);
  AddTestCase (new NgcN2uDlTestCase ("2 eNBs", v3), TestCase::QUICK);
  ...
  v4.push_back (e1);
  v4.push_back (e2);
  AddTestCase (new NgcN2uDlTestCase ("3 eNBs", v4), TestCase::QUICK);
  ...
  e5.ues.push_back (f5);
  v5.push_back (e5);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 10 pkts 3000 bytes each", v5), TestCase::QUICK);
  ...
  e6.ues.push_back (f6);
  v6.push_back (e6);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 50 pkts 3000 bytes each", v6), TestCase::QUICK);
  ...
  e7.ues.push_back (f7);
  v7.push_back (e7);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 10 pkts 15000 bytes each", v7), TestCase::QUICK);
  ...
  e8.ues.push_back (f8);
  v8.push_back (e8);
  AddTestCase (new NgcN2uDlTestCase ("1 eNB, 100 pkts 15000 bytes each", v8), TestCase::QUICK);
}
