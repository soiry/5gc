...
my $launch = "CXXFLAGS=\"-O3 -w\" ./waf -d optimized configure --enable-static --enable-examples --enable-modules=nr"
my $out, my $err;
...
foreach my $time (@simTime)
{
         foreach my $node (@nodes)
         {
            ...
            {
               $launch = "time ./waf --run \'lena-simple-ngc --simTime=$time --numberOfNodes=$node'";
               print "$launch\n";
               capture { system($launch ) } \$out, \$err;
               $err =~ /real(.+)m(.+)s/;
               my $minutes = $1;
