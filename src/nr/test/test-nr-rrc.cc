...
NrRrcConnectionEstablishmentTestCase::DoRun ()
{
  ...
  enbDevs = m_nrHelper->InstallEnbDevice (enbNodes);
  stream += m_nrHelper->AssignStreams (enbDevs, stream);

  NetDeviceContainer ueDevs;
  ueDevs = m_nrHelper->InstallUeDevice (ueNodes);
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      //void (NrHelper::* overloadedAttachFunctionPointer) (Ptr<NetDevice>, Ptr<NetDevice>) = &NrHelper::Attach;
      //Simulator::Schedule (MilliSeconds (tc), overloadedAttachFunctionPointer, nrHelper, *it, enbDevice);
      Simulator::Schedule (MilliSeconds (tc), &NrRrcConnectionEstablishmentTestCase::Connect, this, ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc), &NrRrcConnectionEstablishmentTestCase::CheckConnected, this, *it, enbDevice);
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/NrUeRrc/ConnectionEstablished",
                   MakeCallback (&NrRrcConnectionEstablishmentTestCase::ConnectionEstablishedCallback,
                                 this));
  Config::Connect ("/NodeList/*/DeviceList/*/NrUeRrc/ConnectionTimeout",
                   MakeCallback (&NrRrcConnectionEstablishmentTestCase::ConnectionTimeoutCallback,
                                 this));
...
NrRrcConnectionEstablishmentTestCase::Connect (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  ...
  m_nrHelper->Attach (ueDevice, enbDevice);
...
void
NrRrcConnectionEstablishmentTestCase::CheckConnected (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  Ptr<NrUeNetDevice> ueNrDevice = ueDevice->GetObject<NrUeNetDevice> ();
  ...
  NS_TEST_ASSERT_MSG_EQ (ueRrc->GetState (), NrUeRrc::CONNECTED_NORMALLY,
...
void
NrRrcConnectionEstablishmentTestCase::CheckNotConnected (Ptr<NetDevice> ueDevice, Ptr<NetDevice> enbDevice)
{
  Ptr<NrUeNetDevice> ueNrDevice = ueDevice->GetObject<NrUeNetDevice> ();
  Ptr<NrUeRrc> ueRrc = ueNrDevice->GetRrc ();
  const uint64_t imsi = ueNrDevice->GetImsi ();
...
void
NrRrcConnectionEstablishmentTestCase::ConnectionEstablishedCallback (
...
void
NrRrcConnectionEstablishmentTestCase::ConnectionTimeoutCallback (
    std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
  NS_LOG_FUNCTION (this << imsi << cellId);
}
...
class NrRrcConnectionEstablishmentErrorTestCase
  : public NrRrcConnectionEstablishmentTestCase
{
public:
  /**
   *
   ...
   * \param description additional description of the test case
   */
  NrRrcConnectionEstablishmentErrorTestCase (Time jumpAwayTime,
                                              std::string description = "");
protected:
  virtual void DoRun (void);
...
NrRrcConnectionEstablishmentErrorTestCase::NrRrcConnectionEstablishmentErrorTestCase (
  ...
  : NrRrcConnectionEstablishmentTestCase (1, 1, 0, 0, 1, true, false, true,
                                           description),
...
NrRrcConnectionEstablishmentErrorTestCase::DoRun ()
{
  ...
  for (NetDeviceContainer::Iterator it = ueDevs.Begin (); it != ueDevs.End (); ++it)
    {
      ...
      //void (NrHelper::* overloadedAttachFunctionPointer) (Ptr<NetDevice>, Ptr<NetDevice>) = &NrHelper::Attach;
      //Simulator::Schedule (MilliSeconds (tc), overloadedAttachFunctionPointer, nrHelper, *it, enbDevice);
      Simulator::Schedule (MilliSeconds (tc), &NrRrcConnectionEstablishmentErrorTestCase::Connect, this, ueDevice, enbDevice);

      Simulator::Schedule (MilliSeconds (tcc), &NrRrcConnectionEstablishmentErrorTestCase::CheckConnected, this, *it, enbDevice);
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/NrUeRrc/ConnectionEstablished",
                   MakeCallback (&NrRrcConnectionEstablishmentErrorTestCase::ConnectionEstablishedCallback,
                                 this));
  Config::Connect ("/NodeList/*/DeviceList/*/NrUeRrc/ConnectionTimeout",
                   MakeCallback (&NrRrcConnectionEstablishmentErrorTestCase::ConnectionTimeoutCallback,
                                 this));
  ...
  for (uint32_t useIdealRrc = 0; useIdealRrc <= 1; ++useIdealRrc)
    {
      ...
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     0,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     0,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     1,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     1,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     2,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     2,    100,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     0,     20,          10,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     0,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     1,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     1,     20,          10,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     1,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     2,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     2,     20,          10,           1, false, useIdealRrc, true), TestCase::QUICK);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     2,     20,         100,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  3,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  4,     0,     20,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  4,     0,     20,         300,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase ( 20,     0,     10,           1,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase ( 50,     0,      0,           0,           1, false, useIdealRrc, true), TestCase::EXTENSIVE);

      // Test cases to check admitRrcConnectionRequest=false
      //                                                     nUes      tConnBase            delayDiscStart         useIdealRrc
      //                                                        nBearers       tConnIncrPerUe       errorExpected               admitRrcConnectionRequest
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     0,      0,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  1,     2,    100,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     0,     20,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  2,     1,     20,           0,           1, false, useIdealRrc, false), TestCase::QUICK);
      AddTestCase (new NrRrcConnectionEstablishmentTestCase (  3,     0,     20,           0,           1, false, useIdealRrc, false), TestCase::EXTENSIVE);
    }
