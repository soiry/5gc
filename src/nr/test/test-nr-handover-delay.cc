...
{
public:
  NrHandoverDelayTestCase (bool useIdealRrc, Time handoverTime,
...
NrHandoverDelayTestCase::DoRun ()
{
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/NrEnbRrc/HandoverStart",
      MakeCallback (&NrHandoverDelayTestCase::EnbHandoverStartCallback, this));
  Config::Connect ("/NodeList/*/DeviceList/*/NrEnbRrc/HandoverEndOk",
      MakeCallback (&NrHandoverDelayTestCase::EnbHandoverEndOkCallback, this));

  // SIMULATION

  nrHelper->AddX2Interface (enbNodes);
  nrHelper->Attach (ueDevs.Get(0), enbDevs.Get(0));
  nrHelper->HandoverRequest (m_handoverTime, ueDevs.Get (0), enbDevs.Get (0),
      enbDevs.Get (1));
  // disable error model in dl ctrl channel
  //Config::Set ("/NodeList/*/DeviceList/*/NrUePhy/DlSpectrumPhy/CtrlErrorModelEnabled",
  //    BooleanValue (false));
  Simulator::Stop (m_simulationDuration);
  Simulator::Run ();
  Simulator::Destroy ();

} // end of void NrHandoverDelayTestCase::DoRun ()
...
void
NrHandoverDelayTestCase::EnbHandoverStartCallback (std::string context,
    uint64_t imsi, uint16_t cellid, uint16_t rnti, uint16_t targetCellId)
{
  NS_LOG_FUNCTION (this << context);
...
void
NrHandoverDelayTestCase::EnbHandoverEndOkCallback (std::string context,
    uint64_t imsi, uint16_t cellid, uint16_t rnti)
{
  NS_LOG_FUNCTION (this << context);
...
public:
  NrHandoverDelayTestSuite ()
      : TestSuite ("nr-handover-delay", TestSuite::SYSTEM)
  {
    ...
    for (Time handoverTime = Seconds (0.100); handoverTime < Seconds (0.110);
        handoverTime += Seconds (0.001))
      {
        ...
        AddTestCase (
            new NrHandoverDelayTestCase (true, handoverTime, Seconds (0.005),
    ...
    for (Time handoverTime = Seconds (0.100); handoverTime < Seconds (0.110);
        handoverTime += Seconds (0.001))
      {
        // arguments: useIdealRrc, handoverTime, delayThreshold, simulationDuration
        AddTestCase (
            new NrHandoverDelayTestCase (false, handoverTime, Seconds (0.020),
                Seconds (0.200)), TestCase::QUICK);
      }
  }
} g_nrHandoverDelayTestSuite;
