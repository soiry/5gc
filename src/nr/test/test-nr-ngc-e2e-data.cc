...
NS_LOG_COMPONENT_DEFINE ("NrNgcE2eData");
...
class NrNgcE2eDataTestCase : public TestCase
{
public:
  NrNgcE2eDataTestCase (std::string name, std::vector<EnbTestData> v);
  virtual ~NrNgcE2eDataTestCase ();
...
NrNgcE2eDataTestCase::NrNgcE2eDataTestCase (std::string name, std::vector<EnbTestData> v)
  : TestCase (name),
    m_enbTestData (v)
{
...
NrNgcE2eDataTestCase::~NrNgcE2eDataTestCase ()
{
}

void 
NrNgcE2eDataTestCase::DoRun ()
{
  ...
  // allow jumbo frames on the N2-U link
  ngcHelper->SetAttribute ("N2uLinkMtu", UintegerValue (30000));
  ...
  NetDeviceContainer enbNrDevs = nrHelper->InstallEnbDevice (enbs);
  NetDeviceContainer::Iterator enbNrDevIt = enbNrDevs.Begin ();
  ...
  for (std::vector<EnbTestData>::iterator enbit = m_enbTestData.begin ();
       ...
       ++enbit, ++enbNrDevIt)
    {
      NS_ABORT_IF (enbNrDevIt ==  enbNrDevs.End ());
      


      NodeContainer ues;
      ues.Create (enbit->ues.size ());
      Vector enbPosition = (*enbNrDevIt)->GetNode ()->GetObject<MobilityModel> ()->GetPosition ();
      ...
      NetDeviceContainer ueNrDevs = nrHelper->InstallUeDevice (ues);
      ...
      for (uint32_t u = 0; u < ues.GetN (); ++u)
        {
          ...
          nrHelper->Attach (ueNrDevice, *enbNrDevIt);        
  ...
  nrHelper->GetPdcpStats ()->SetAttribute ("StartTime", TimeValue (Seconds (statsStartTime)));
  nrHelper->GetPdcpStats ()->SetAttribute ("EpochDuration", TimeValue (Seconds (statsDuration)));
          ...
          for (uint32_t b = 0; b < ueit->bearers.size (); ++b)
            {
              ...
              uint32_t txPktsPdcpDl = nrHelper->GetPdcpStats ()->GetDlTxPackets (imsi, lcid);
              uint32_t rxPktsPdcpDl = nrHelper->GetPdcpStats ()->GetDlRxPackets (imsi, lcid);
              uint32_t txPktsPdcpUl = nrHelper->GetPdcpStats ()->GetUlTxPackets (imsi, lcid);
              uint32_t rxPktsPdcpUl = nrHelper->GetPdcpStats ()->GetUlRxPackets (imsi, lcid);
...
/**
 * Test that the N2-U interface implementation works correctly
 */
class NrNgcE2eDataTestSuite : public TestSuite
{
public:
  NrNgcE2eDataTestSuite ();
  
} g_nrNgcE2eDataTestSuite;

NrNgcE2eDataTestSuite::NrNgcE2eDataTestSuite ()
  : TestSuite ("nr-ngc-e2e-data", SYSTEM)
{  
  ...
  e1.ues.push_back (u1);
  v1.push_back (e1);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE", v1), TestCase::QUICK);
  ...
  v2.push_back (e2);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 2UEs", v2), TestCase::EXTENSIVE);

  std::vector<EnbTestData> v3;  
  v3.push_back (e1);
  v3.push_back (e2);
  AddTestCase (new NrNgcE2eDataTestCase ("2 eNBs", v3), TestCase::EXTENSIVE);
  ...
  v4.push_back (e1);
  v4.push_back (e2);
  AddTestCase (new NrNgcE2eDataTestCase ("3 eNBs", v4), TestCase::EXTENSIVE);
  ...
  v5.push_back (e5);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE with 1000 byte packets", v5), TestCase::EXTENSIVE);
  ...
  v6.push_back (e6);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE with 1400 byte packets", v6), TestCase::EXTENSIVE);
  ...
  v7.push_back (e7);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE with 2 bearers", v7), TestCase::EXTENSIVE);
  ...
  v8.push_back (e8);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE with fragmentation", v8), TestCase::EXTENSIVE);
  ...
  v9.push_back (e9);
  AddTestCase (new NrNgcE2eDataTestCase ("1 eNB, 1UE with aggregation", v9), TestCase::EXTENSIVE);
