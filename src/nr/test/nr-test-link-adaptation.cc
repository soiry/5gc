...
#include "nr-test-link-adaptation.h"


using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("NrLinkAdaptationTest");
...
void
NrTestDlSchedulingCallback (NrLinkAdaptationTestCase *testcase, std::string path,
...
NrLinkAdaptationTestSuite::NrLinkAdaptationTestSuite ()
  : TestSuite ("nr-link-adaptation", SYSTEM)
{
  NS_LOG_INFO ("Creating NrLinkAdaptionTestSuite");
  ...
  for ( int i = 0 ; i < numOfTests; i++ )
    {
      ...
      AddTestCase (new NrLinkAdaptationTestCase (name.str (),  snrEfficiencyMcs[i].snrDb, lossDb, snrEfficiencyMcs[i].mcsIndex), TestCase::QUICK);
    }

}

static NrLinkAdaptationTestSuite nrLinkAdaptationTestSuite;
...
NrLinkAdaptationTestCase::NrLinkAdaptationTestCase (std::string name, double snrDb, double loss, uint16_t mcsIndex)
  : TestCase (name),
    m_snrDb (snrDb),
    m_loss (loss),
    m_mcsIndex (mcsIndex)
{
  ...
  NS_LOG_INFO ("Creating NrLinkAdaptationTestCase: " + sstream1.str ());
}

NrLinkAdaptationTestCase::~NrLinkAdaptationTestCase ()
{
}

void
NrLinkAdaptationTestCase::DoRun (void)
{
  Config::Reset ();
  Config::SetDefault ("ns3::NrAmc::AmcModel", EnumValue (NrAmc::PiroEW2010));
  Config::SetDefault ("ns3::NrAmc::Ber", DoubleValue (0.00005));
  Config::SetDefault ("ns3::NrEnbRrc::SrsPeriodicity", UintegerValue (2));
...
//   nrHelper->EnableLogComponents ();
  ...
  nrHelper->SetPathlossModelAttribute ("Loss", DoubleValue (m_loss));
...
void
NrLinkAdaptationTestCase::DlScheduling (uint32_t frameNo, uint32_t subframeNo, uint16_t rnti,
                                         uint8_t mcsTb1, uint16_t sizeTb1, uint8_t mcsTb2, uint16_t sizeTb2) 
{
