...
static ns3::GlobalValue g_ngc ("ngc",
                               ...
                               "If false, only the NR radio access will be simulated with RLC SM. ",
                               ns3::BooleanValue (false),
                               ns3::MakeBooleanChecker ());
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::NrEnbRrc::SrsPeriodicity", UintegerValue (srsPeriodicity));
  ...
  nrHelper->SetPathlossModelAttribute ("ShadowSigmaExtWalls", DoubleValue (0));
  nrHelper->SetPathlossModelAttribute ("ShadowSigmaOutdoor", DoubleValue (1));
  nrHelper->SetPathlossModelAttribute ("ShadowSigmaIndoor", DoubleValue (1.5));
  // use always LOS model
  nrHelper->SetPathlossModelAttribute ("Los2NlosThr", DoubleValue (1e6));
...
//   LogComponentEnable ("NrPfFfMacScheduler", LOG_LEVEL_ALL);
  ...
  Ptr<NrHexGridEnbTopologyHelper> nrHexGridEnbTopologyHelper = CreateObject<NrHexGridEnbTopologyHelper> ();
  nrHexGridEnbTopologyHelper->SetNrHelper (nrHelper);
  nrHexGridEnbTopologyHelper->SetAttribute ("InterSiteDistance", DoubleValue (interSiteDistance));
  nrHexGridEnbTopologyHelper->SetAttribute ("MinX", DoubleValue (interSiteDistance/2));
  nrHexGridEnbTopologyHelper->SetAttribute ("GridWidth", UintegerValue (nMacroEnbSitesX));
  Config::SetDefault ("ns3::NrEnbPhy::TxPower", DoubleValue (macroEnbTxPowerDbm));
  nrHelper->SetEnbAntennaModelType ("ns3::ParabolicAntennaModel");
  nrHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (70));
  nrHelper->SetEnbAntennaModelAttribute ("MaxAttenuation",     DoubleValue (20.0));
  nrHelper->SetEnbDeviceAttribute ("DlEarfcn", UintegerValue (macroEnbDlEarfcn));
  nrHelper->SetEnbDeviceAttribute ("UlEarfcn", UintegerValue (macroEnbDlEarfcn + 18000));
  nrHelper->SetEnbDeviceAttribute ("DlBandwidth", UintegerValue (macroEnbBandwidth));
  nrHelper->SetEnbDeviceAttribute ("UlBandwidth", UintegerValue (macroEnbBandwidth));
  NetDeviceContainer macroEnbDevs = nrHexGridEnbTopologyHelper->SetPositionAndInstallEnbDevice (macroEnbs);

  if (ngc)
    {
      // this enables handover for macro eNBs
      nrHelper->AddX2Interface (macroEnbs);
    }
  ...
  Config::SetDefault ("ns3::NrEnbPhy::TxPower", DoubleValue (homeEnbTxPowerDbm));
  nrHelper->SetEnbAntennaModelType ("ns3::IsotropicAntennaModel");
  nrHelper->SetEnbDeviceAttribute ("DlEarfcn", UintegerValue (homeEnbDlEarfcn));
  nrHelper->SetEnbDeviceAttribute ("UlEarfcn", UintegerValue (homeEnbDlEarfcn + 18000));
  nrHelper->SetEnbDeviceAttribute ("DlBandwidth", UintegerValue (homeEnbBandwidth));
  nrHelper->SetEnbDeviceAttribute ("UlBandwidth", UintegerValue (homeEnbBandwidth));
  nrHelper->SetEnbDeviceAttribute ("CsgId", UintegerValue (1));
  nrHelper->SetEnbDeviceAttribute ("CsgIndication", BooleanValue (true));
  NetDeviceContainer homeEnbDevs  = nrHelper->InstallEnbDevice (homeEnbs);
  ...
  NetDeviceContainer homeUeDevs = nrHelper->InstallUeDevice (homeUes);
  ...
  NetDeviceContainer macroUeDevs = nrHelper->InstallUeDevice (macroUes);
  ...
  if (ngc)
    {
      ...
      nrHelper->Attach (macroUeDevs);
      nrHelper->Attach (homeUeDevs);
    }
  else
    {
      // macro UEs attached to the closest macro eNB
      nrHelper->AttachToClosestEnb (macroUeDevs, macroEnbDevs);
      ...
      for (ueDevIt = homeUeDevs.Begin (), enbDevIt = homeEnbDevs.Begin ();
           ueDevIt != homeUeDevs.End (); ++ueDevIt, ++enbDevIt)
        {
          ...
          nrHelper->Attach (*ueDevIt, *enbDevIt);
        }
    }
          ...
          for (uint32_t b = 0; b < numBearersPerUe; ++b)
            {
              ...
              EpsBearer bearer (q);
              nrHelper->ActivateDataRadioBearer (ueDev, bearer);
            }
        }
    }
