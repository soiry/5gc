...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::NrEnbPhy::TxPower", DoubleValue (20.0));
  enbDevs = nrHelper->InstallEnbDevice (oneSectorNodes);


  // power setting for three-sector macrocell
  Config::SetDefault ("ns3::NrEnbPhy::TxPower", DoubleValue (43.0));
  ...
  nrHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (100));
  nrHelper->SetEnbAntennaModelAttribute ("MaxGain",     DoubleValue (0.0));
  enbDevs.Add ( nrHelper->InstallEnbDevice (threeSectorNodes.Get (0)));

  nrHelper->SetEnbAntennaModelType ("ns3::CosineAntennaModel");
  nrHelper->SetEnbAntennaModelAttribute ("Orientation", DoubleValue (360/3));
  nrHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (100));
  nrHelper->SetEnbAntennaModelAttribute ("MaxGain",     DoubleValue (0.0));
  enbDevs.Add ( nrHelper->InstallEnbDevice (threeSectorNodes.Get (1)));

  nrHelper->SetEnbAntennaModelType ("ns3::CosineAntennaModel");
  nrHelper->SetEnbAntennaModelAttribute ("Orientation", DoubleValue (2*360/3));
  nrHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (100));
  nrHelper->SetEnbAntennaModelAttribute ("MaxGain",     DoubleValue (0.0));
  enbDevs.Add ( nrHelper->InstallEnbDevice (threeSectorNodes.Get (2)));

  for (uint32_t i = 0; i < nEnb; i++)
    {
      NetDeviceContainer ueDev = nrHelper->InstallUeDevice (ueNodes.at(i));
      ueDevs.push_back (ueDev);
      nrHelper->Attach (ueDev, enbDevs.Get (i));
