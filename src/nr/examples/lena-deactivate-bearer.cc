...
/**
 * Sample simulation script for NR+NGC. It instantiates one eNodeB,
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("NgcAmf", logLevel);
  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      Ptr<NetDevice> ueDevice = ueNrDevs.Get (u);
  ...
  Ptr<NetDevice> ueDevice = ueNrDevs.Get (0);
  Ptr<NetDevice> enbDevice = enbNrDevs.Get (0);
  ...
  Simulator::Schedule (deActivateTime, &NrHelper::DeActivateDedicatedEpsBearer, nrHelper, ueDevice, enbDevice, 2);
