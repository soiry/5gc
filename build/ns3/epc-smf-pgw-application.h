...
#ifndef EPC_SMF_PGW_APPLICATION_H
#define EPC_SMF_PGW_APPLICATION_H
 ...
 * \ingroup lte
 *
 * This application implements the SMF/PGW functionality.
 */
class EpcSmfPgwApplication : public Application
{
  friend class MemberEpcS11SapSmf<EpcSmfPgwApplication>;
...
public:
   ...
   * the Gi interface of the PGW/SMF over the
   * internet over GTP-U/UDP/IP on the S1-U interface
   * \param s1uSocket socket used to send GTP-U packets to the eNBs
   */
  EpcSmfPgwApplication (const Ptr<VirtualNetDevice> tunDevice, const Ptr<Socket> s1uSocket);

  /** 
   * Destructor
   */
  virtual ~EpcSmfPgwApplication (void);
  
  /** 
   * Method to be assigned to the callback of the Gi TUN VirtualNetDevice. It
   * is called when the SMF/PGW receives a data packet from the
   ...
   * is called when the SMF/PGW receives a data packet from the eNB
  ...
  /** 
   * Send a packet to the internet via the Gi interface of the SMF/PGW
   * 
   * \param packet 
   */
  ...
  /** 
   * 
   * \return the SMF side of the S11 SAP 
   */
  EpcS11SapSmf* GetS11SapSmf ();
...
private:
  ...
  // S11 SAP SMF methods
  void DoCreateSessionRequest (EpcS11SapSmf::CreateSessionRequestMessage msg);
  void DoModifyBearerRequest (EpcS11SapSmf::ModifyBearerRequestMessage msg);  

  void DoDeleteBearerCommand (EpcS11SapSmf::DeleteBearerCommandMessage req);
  void DoDeleteBearerResponse (EpcS11SapSmf::DeleteBearerResponseMessage req);
...
public:
    ...
    /** 
     * \brief Function, deletes contexts of bearer on SMF and PGW side
  ...
  */
  ...
  /**
   * SMF side of the S11 SAP
   * 
   */
  EpcS11SapSmf* m_s11SapSmf;
...
#endif /* EPC_SMF_PGW_APPLICATION_H */
